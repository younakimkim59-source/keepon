...
function handleJoinFamily() {
    ...
    if (code.length < 4) {
        alert('가족 코드는 4자 이상이어야 합니다.');
        return;
    }
    
    currentFamilyCode = code;
    displayFamilyCode.textContent = code;
...
function handleCreateFamily() {
    ...
    familyCodeInput.value = newCode;
    currentFamilyCode = newCode;
    displayFamilyCode.textContent = newCode;
...
function handleLogout() {
    if (confirm('가족 지갑에서 로그아웃하시겠습니까?')) {
        ...
        localStorage.removeItem(`gifts_${currentFamilyCode}`);
        currentFamilyCode = '';
        gifts = [];
        loginModal.style.display = 'block';
        mainContainer.style.display = 'none';
